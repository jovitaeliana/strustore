...
DISCORD_ALERT_WEBHOOK_URL = "https://discord.com/api/webhooks/1400873533915005039/-1CXFn6A_rlrBV-2Xyiv9erlbQBlAXxrq-d5jIc4cpWEWRx0P1jt5a8OPH0ok2_UyWJg"

def send_discord_alert(title, description, severity="error", marketplace="GroundingDINO", details=None):
    if not DISCORD_ALERT_WEBHOOK_URL:
        print("Discord webhook URL not configured. Skipping alert.")
        return

    color = 0xff0000 if severity == "error" else (0xffa500 if severity == "warning" else 0x00ff00)
    
    embed = {
        "title": f"🚨 {marketplace} Alert: {title}",
    ...
    try:
        response = requests.post(DISCORD_ALERT_WEBHOOK_URL, json=payload)
...
#     send discord alert
    send_discord_alert(
        ...
        description=f"Training finished successfully in {total_time_str}.",
        severity="info",
        marketplace="GroundingDINO",
        details={
